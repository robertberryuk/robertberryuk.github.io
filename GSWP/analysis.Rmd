...
**Methodology for AES calculations**

The following datasets (ESRI shapefile format) on agri-environment payments were downloaded on 28/01/2022 from the [Natural England Open Data Geoportal](https://naturalengland-defra.opendata.arcgis.com/):

- **Environmental Stewardship (ES)**: "Environmental_Stewardship_Scheme_Agreements.shp" <br />
- **Countryside Stewardship (CS)**: "Countryside_Stewardship_Scheme_Management_Areas.shp" <br />

These data differ to the P1 data in that ES and CS agreements are "point" data, which are geographically referenced using a precise x,y coordinate. Each point represents the geographical centre of each ES/CS agreement. This means that extracting agreements which fall within a specific boundary is more straightforward then with the P1 data - which is aggregated to postcode districts. The basic steps of the workflow as follows:<br />
- **Step 1**: Initiate a programming loop based on the target boundaries (i.e. counties, national parks) <br />
- **Step 2**: Clip AES data layer to the geographical extent of the target boundary  <br />
- **Step 3**: For each boundary area, calculate the total AES payments for each year represented in the data layer. Note, the approaches for calculating annual totals ES and CS differ due to the structural differences of the datasets. This is quite complex and is fully documented in the R code.<br />
- **Step 4**: Display output tables and bar plots  <br />
- **Step 5**: Export output tables and bar plots to CCRI shared drive  <br />
<br />
<br />
<br />
...
County-level analysis of Environmental Stewardship (ES) payments. 
