...
Calculation of P1 payment reductions at sub-county (district unitary authority) level. Uses same basic methodology as above, but the level of data management complexity is higher.
...
<!-- #> Practice ggplot for district -1 Payments -->

<!-- data <- read_csv(here("Out", "P1", "District", "CSV", "P1_Reductions_Devon.csv")) -->

<!-- districts.list <- as.list(data$Name) -->
...
<!-- plot2 <- ggplot(p2.plot, aes(x=Year, y=Value_bps)) +  -->
<!--   theme_bw() + -->
<!--   geom_bar(stat = "identity", fill="#599ad3", width = 0.6) + -->
<!--   geom_text(aes(label= paste0(round(Value_bps / 1000000, digits = 2), " M")), vjust = 1.5, colour = "white") + -->
<!--   labs(title = paste0("Pillar 1 reductions:", " ", district.active), x = "Year", y = "Value (£)") + -->
<!--   theme(plot.title = element_text(hjust = 0.5)) + -->
<!--   theme(axis.title.y = element_text(margin = margin(t = 0, r = 16, b = 0, l = 0))) + -->
